...
alphabet = ['а', 'б', 'в', 'г', 'д', 'е', 'ж', 'з', 'и', 'й', 'к', 'л', 'м', 'н', 'о', 'п', 'р', 'с', 'т', 'у', 'ф', 'х'
    , 'ц', 'ч', 'ш', 'щ', 'ъ', 'ы', 'ь', 'э', 'ю', 'я']
word_before = input() + ' запретил букву'
word_after = ''
index_alphabet = 0
while len(word_before.replace(" ", '')) != 0:
    if alphabet[index_alphabet] in word_before:
        print(word_before, alphabet[index_alphabet])
        for char in word_before:
            if char != alphabet[index_alphabet]:
                word_after += char
        word_before = word_after.strip().replace('  ', ' ')
        word_after = ''
        index_alphabet += 1
    else:
        index_alphabet += 1
