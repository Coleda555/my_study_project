...
with open('dataset_3380_5.txt') as inf:
    for line in inf:
        list_all.append([word for word in line.split()])

for i in range(1, 12):
    counter = 0
    list_class = []
    for class_room in list_all:
        if i == int(class_room[0]):
            counter += 1
            list_class.append(float(class_room[2]))
    if counter > 0:
        medium = 0
        for height in list_class:
            medium += float(height)
        print(i, medium / counter)
    else:
        print(i, '-')
