...
Обратите внимание при создании объекта класса MotherBoard можно передавать несколько объектов класса Memory, максимум
N - по числу слотов памяти на материнской плате (по умолчанию N = 4).
...
class MotherBoard:
    ...
    def __init__(self, name, cpu, *args, total_mem_slots=4):
        self.name = name
        self.cpu = cpu
        self.total_mem_slots = total_mem_slots
        self.mem_slots = args
        if len(self.mem_slots) > self.total_mem_slots:
            self.mem_slots = self.mem_slots[:self.total_mem_slots]
    ...
    def get_config(self):
        return [
            ...
            'Память:' + ','.join(
                map(lambda memory: ' ' + str(memory.name) + ' - ' + str(memory.volume), self.mem_slots)
            )
        ]


mb = MotherBoard('Mother', CPU('cpu1', 50), Memory('memory1', 2), Memory('memory2', 4), Memory('memory3', 6),
                 Memory('memory4', 8), Memory('memory5', 10), total_mem_slots=5)
