...
n = int(input())
for _ in range(n):
    list_input = [k for k in input().split()]
    if list_input[0] not in dict_input:
        dict_input[list_input[0]] = dict()
        dict_input[list_input[0]][list_input[1]] = [int(list_input[2])]
    else:
        if list_input[1] not in dict_input[list_input[0]]:
            dict_input[list_input[0]][list_input[1]] = [int(list_input[2])]
        else:
            dict_input[list_input[0]][list_input[1]].append(int(list_input[2]))

for key, value in sorted(dict_input.items()):
    print('{0}:'.format(key))
    for item, cost in sorted(value.items()):
        print(item, sum(cost))
