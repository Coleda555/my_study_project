...
with open('logfile.txt', 'r', encoding='utf-8') as file_input:
    data = list(map(lambda line: line.split(','), file_input.readlines()))
    text_output = []
    for line in data:
        enter = line[1].split(':')
        enter_min = int(enter[0]) * 60 + int(enter[1])
        exit = line[2].split(':')
        exit_min = int(exit[0]) * 60 + int(exit[1])
        result = exit_min - enter_min
        if result >= 60:
            text_output.append(line[0] + '\n')

with open('output.txt', 'w', encoding='utf-8') as file_output:
    file_output.writelines(text_output)
