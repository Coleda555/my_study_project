...
P.S. В программе требуется объявить классы с описанным функционалом. На эк
"""
...
class Bag:
    ...
    def add_thing(self, thing):
        if sum(map(lambda item: item.weight, self.__things)) + thing.weight <= self.max_weight:
    ...
    def get_total_weight(self):
        return sum(map(lambda item: item.weight, self.__things))
